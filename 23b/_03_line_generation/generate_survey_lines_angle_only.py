...
"""
仅角度修正的测线长度脚本
基于现有成功的测线生成，对测线长度仅进行角度修正延伸优化
去除基础延伸和安全边距，仅保留角度修正部分
"""
...
def find_line_region_intersections_angle_only(x_start, y_start, x_end, y_end, 
                                            x_min, x_max, y_min, y_max, depth_interpolator):
    """
    计算测线与矩形区域边界的交点，并仅根据角度修正进行延伸
    去除基础延伸量和安全边距，仅保留角度修正部分
    """
    # 首先找到基本的边界交点
    ...
    # 在交点处采样深度，计算需要的延伸距离
    def get_extension_distance_angle_only(x, y, boundary_type):
        """仅计算角度修正延伸距离 - 去除基础延伸和安全边距"""
        # 查询该点的深度
        ...
        # 仅角度修正策略：
        # 只保留角度修正量，去除基础延伸和安全边距
        angle_extension = swath_width * (angle_factor - 1) * 0.5  # 角度修正
        
        # 仅返回角度修正延伸量
        total_extension = angle_extension
        
        return total_extension
    
    # 计算起点延伸距离
    boundary_type_start = intersections[0][3]
    ext_dist_start = get_extension_distance_angle_only(x1, y1, boundary_type_start)
    
    # 计算终点延伸距离  
    boundary_type_end = intersections[-1][3]
    ext_dist_end = get_extension_distance_angle_only(x2, y2, boundary_type_end)
...
def optimize_survey_lines(lines_df, region_boundaries, depth_interpolator):
    ...
    for _, line in lines_df.iterrows():
        ...
        # 对测线仅进行角度修正延伸
        result = find_line_region_intersections_angle_only(
        ...
        if result is not None:
            ...
            optimized_line['length_angle_only_nm'] = optimized_length
...
def visualize_survey_plan(lines_df, region_boundaries, grid_data, 
                         output_path="survey_plan_angle_only.png"):
    """创建仅角度修正的测线方案可视化图."""
    ...
    ax.set_title('仅角度修正测线方案', fontsize=16, weight='bold')
...
def main():
    """Main function to run the angle-only optimized survey line planning pipeline."""
    # 1. Load data
    print("正在加载数据...")
    try:
    ...
    # 2. Optimize survey lines (angle-only)
    print("正在进行仅角度修正的测线长度优化...")
    lines_df_optimized = optimize_survey_lines(lines_df_original, region_boundaries_df, depth_interpolator)
    lines_df_optimized.to_csv("survey_lines_angle_only.csv", index=False, float_format='%.4f')
    print("仅角度修正的测线数据已保存至 survey_lines_angle_only.csv")

    # 3. Generate optimization report
    print("正在生成优化报告...")
    total_length = lines_df_optimized['length_angle_only_nm'].sum()

    report_content = "# 仅角度修正测线方案报告\n\n"
    report_content += "## 优化策略说明\n\n"
    report_content += "采用极简的角度修正延伸算法，仅根据测线与边界的入射角度进行适当延伸。"
    report_content += "完全去除基础延伸量和安全边距，仅保留必要的角度修正部分。\n\n"
    report_content += "**核心策略**：\n"
    report_content += "- 去除基础延伸距离（原为完整覆盖宽度）\n"
    report_content += "- 去除安全边距（原为20%覆盖宽度）\n" 
    report_content += "- 仅保留角度修正：angle_extension = swath_width × (angle_factor - 1) × 0.5\n"
    report_content += "- 极简延伸策略，最小化测线长度\n\n"
    ...
    for region_id in lines_df_optimized['region_id'].unique():
        ...
        length_total = region_lines['length_angle_only_nm'].sum()
    ...
    report_content += "**技术特点**: \n"
    report_content += "1. 仅根据入射角度进行必要延伸\n"
    report_content += "2. 最小化测线长度和作业成本\n"
    report_content += "3. 保持测线平行性\n"
    report_content += "4. 极简延伸策略\n\n"
    
    with open("survey_angle_only_report.md", "w", encoding='utf-8') as f:
        f.write(report_content)
    print("优化报告已生成: survey_angle_only_report.md")
