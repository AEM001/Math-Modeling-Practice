...
class Visualizer:
    ...
    def set_text_properties(self, text_obj):
        """设置文本对象的中文字体属性"""
        if hasattr(text_obj, 'set_fontfamily'):
            text_obj.set_fontfamily(self.chinese_font)
        return text_obj
    
    def configure_axes_fonts(self, ax):
        """配置坐标轴的中文字体"""
        # 设置标题字体
        if ax.get_title():
            ax.title.set_fontfamily(self.chinese_font)
        
        # 设置轴标签字体
        ax.xaxis.label.set_fontfamily(self.chinese_font)
        ax.yaxis.label.set_fontfamily(self.chinese_font)
